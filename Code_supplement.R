...
dn_mom <- c("GPD_GML_extRemes", "GPD_LMO_lmomco", "GPD_LMO_extRemes", 
...
source("Code_aid.R"); aid$load("PT"); library(parallel) ; library(extremeStat)
...
trqL_stats <- pblapply(seq_along(PT), cl=cl, FUN=function(stat)
  ...
  file=paste0("sim_ssd_logs/truncstat",stat,".txt") ))
...
dn <- c("GPD_MLE_evd", "GPD_MLE_extRemes", "GPD_MLE_ismev", "GPD_MLE_Renext_Renouv",
        "GPD_MLE_evir", "GPD_MLE_fExtremes", "GPD_PWM_fExtremes",  "GPD_PWM_evir", 
        "GPD_GML_extRemes", "GPD_LMO_extRemes", "GPD_LMO_lmomco", "wak",
        "wei","pe3","gno","gum")
bg <- c(rep("seashell",6), rep("mistyrose",4), "thistle", rep("lavender",5))
names(bg) <- dn
...
par(mfrow=c(4,4), oma=c(2,2.9,0.2,0.4), mar=c(0,0,0,0), xpd=F, mgp=c(1.8,0.5,0), cex=1)
for(d in dn)
{
...
rm(u)
if(d %in% dn[c( 1,13)]) logAxis(2) else logAxis(2, labels=FALSE)
if(d %in% dn[c(13,16)]) axis(1, 0:5*0.2, c("0",1:4*0.2,"1"))
abline(v=0.8, col=8)
...
aid$PTplot(prob="99.9%", outer=TRUE, line=0.2, ylim=c(5,120), main="", cc=FALSE)
